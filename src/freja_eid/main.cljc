(ns freja_eid.main
  "Kotodama WASM entrypoint for the freja_eid clean-room actor (L4) — Clojure port.
  ...
  facts (`freja_eid.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "freja_eid")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"freja_eid.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "freja_eid-compat" :tier tier :entities entities} 200])
...
(defn start! [] :freja_eid-compat/ready)
